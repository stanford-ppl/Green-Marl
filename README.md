...
-------------------------------------
  ...
  The following steps compiles sample Green-Marl programs into C++:
    * cd to (top)/apps
    * check the content of the file "Programs.mk"
    ...
    * check (top)/output_cpp/generated/ and see *.cc files are successfully
        generated.
...
-------------------------------------
  ...
  steps:
    * cd to (top)/apps/
    * 'make bin'
     check if executables are successfully generated at
