...
-------------------------------------
  ...
  The following steps build up 'gm_comp', a Green-Marl compiler:
    ...
    * cd to (top)/src
    * 'make'
    * check if (top)/bin/gm_comp has been created successfully.
    * cd to (top)/bin and type 'gm_comp -h' to look at compiler options

  A more detailed documentation about the gm_comp compiler can be found in
  etc/compiler.txt in this package
...
-------------------------------------
  ...
  through the following steps:
    ...
    * copy etc/vim/syntax/greenmarl.vim into ~/.vim/syntax/
    * copy etc/vim/ftdetect/greenmarl.vim into ~/.vim/ftdetect/
...
-------------------------------------
  ...
  The following steps compiles sample Green-Marl programs into C++:
    ...
    * cd to (top)/apps
    * check the content of the file "Programs.mk"this file contains the names of Green-Marl programs to be compiled (without any file extension).
    * 'make gen'
       this invokes (top)/bin/gm_comp and creates c++ implementation (.cc, .h file) from each Green-Marl program (.gm file).
    * check (top)/output_cpp/generated/ and see *.cc files are successfully generated.
...
-------------------------------------
  ...
  gm_graph library can be built through the following steps:
    ...
    * cd to (top)/apps/
    * 'make lib'
    * check libgmgraph.a has been successfully created in 
...
-------------------------------------
  ...
  * cd to (top)/apps/
  * 'make bin'
    check if executables are successfully generated at (top)/apps/output_cpp/bin
...
-------------------------------------
  ...
  [TODO] 
  (Make graph_gen as a separate package; it is polluted by GPL from GT.)
  ...
  Now, do the following steps:
    ...
    * The remaining programs (pagerank, bc, kosaraju) have the same command-line
        argument format.
...
====================================
  ...
  To see how gm_comp-GPS works, try following steps: 
     ...
     * Check if (top)/apps/output_gps/generate/*.java has been created successfully.
