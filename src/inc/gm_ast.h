...
class ast_node {
    ...
    public:
        ...
        int get_nodetype() {return nodetype;}
        ast_node* get_parent() {return parent;}
        void set_parent(ast_node* p) {parent = p;}
        
        virtual bool is_sentence() {return false;} 
        virtual bool is_expr() {return false;} 
        bool has_symtab() {
            return
            (nodetype == AST_SENTBLOCK) ||
            (nodetype == AST_FOREACH) ||
            (nodetype == AST_PROCDEF) ||
            (nodetype == AST_EXPR_RDC) || 
            (nodetype == AST_BFS) ||
            false ;
        }
        ...
        virtual void traverse(gm_apply*a, bool is_post) {assert(false);}
        virtual void apply_symtabs(gm_apply*a) {assert(false);}


        // scoped elements
        virtual bool has_scope() {return false;}
        virtual gm_symtab* get_symtab_var() {assert(has_scope()); return sym_vars;}
        virtual gm_symtab* get_symtab_field() {assert(has_scope()); return sym_fields;}
        virtual gm_symtab* get_symtab_proc() {assert(has_scope()); return sym_procs;}
        virtual void get_this_scope(gm_scope* s); 
        virtual void set_symtab_var(gm_symtab* v) {assert(has_scope()); sym_vars = v;}
        virtual void  set_symtab_field(gm_symtab* f) {assert(has_scope()); sym_fields=f;}
        virtual void  set_symtab_proc(gm_symtab* p) {assert(has_scope());  sym_procs=p;}
...
class ast_id : public ast_node {
    public:
        ...
        //-------------------------------------------------
        gm_symtab_entry* getSymInfo() {return info;}
        void setSymInfo(gm_symtab_entry* e, bool is_symtab_entry=false) { info = e; if (!is_symtab_entry) use_names_from_symbol();}
...
class ast_idlist : public ast_node {
    public:
        ...
        virtual void apply_id(gm_apply*a); // defined in traverse.cc
        virtual void reproduce(int id_level); 
        virtual void dump_tree(int id_level); 
        ...
        int get_line() {assert(get_length()>0);return lst[0]->get_line();}
        int get_col() {assert(get_length()>0);return lst[0]->get_col();}
...
class ast_field : public ast_node { // access of node/edge property
    public:
        virtual ~ast_field() {
            delete first;
            delete second;
    ...
    public : 
        ...
        ast_typedecl* getSourceTypeInfo() {return first->getTypeInfo();}

        // type information about field (nodeprop/edgeprop)
        int getTypeSummary() {return second->getTypeSummary();}
        ast_typedecl* getTypeInfo() {return second->getTypeInfo();}

        // type information about target (primitive)
        int getTargetTypeSummary() {return second->getTargetTypeSummary();}
        ast_typedecl* getTargetTypeInfo() {return second->getTargetTypeInfo();}

        inline void set_first(ast_id* f) {
            first = f; f->set_parent(this);
        }
        inline void set_second(ast_id* s) {
            second= s; s->set_parent(this);
        }
...
class ast_typedecl : public ast_node {  // property or type
    ...
    public:
        ...
        bool is_graph()     {return gm_is_graph_type(type_id);}
        bool is_node_property()   {return gm_is_node_property_type(type_id);}
        bool is_edge_property()   {return gm_is_edge_property_type(type_id);}
        bool is_property()        {return gm_is_property_type(type_id);}
        ...
        bool is_numeric()         {return gm_is_numeric_type(type_id);}
        bool is_node_compatible() {return gm_is_node_compatible_type(type_id);}
        bool is_edge_compatible() {return gm_is_edge_compatible_type(type_id);}
...
class ast_sent : public ast_node { 
    ...
    public:
        int get_empty_lines_before() {return eline;}
        void set_empty_lines_before(int t) {eline = t;}
        virtual void traverse(gm_apply*a, bool is_post);
        virtual void traverse_sent(gm_apply*a, bool is_post) {assert(false);};
        virtual bool is_sentence() {return true;} 

        // when to set this variable? (-> should be just before code gen.)
        virtual bool is_under_parallel_execution() {return _par;} 
        virtual void set_under_parallel_execution(bool b) {_par = b;}

    private:
        ast_sent(): ast_node(AST_SENT) {}
        int eline;
        bool _par;
...
class ast_sentblock : public ast_sent {
    ...
    public:
        ...
        virtual void reproduce(int id_level);
        virtual void dump_tree(int id_level); 
        virtual void traverse_sent(gm_apply*a, bool is_post);
        virtual void apply_symtabs(gm_apply*a); 
    ...
    private:
        ast_sentblock():ast_sent(AST_SENTBLOCK)  {create_symtabs();}
...
class ast_argdecl : public ast_node {
    ...
    private: ast_argdecl() : ast_node(AST_ARGDECL),tc_finished(false) {}
    public:
        static ast_argdecl* new_argdecl(ast_idlist* id, ast_typedecl* type) {
            ast_argdecl* d = new ast_argdecl(); 
            d->idlist = id; d->type = type;
            id->set_parent(d); type->set_parent(d);
            return d;
        }
        virtual void reproduce(int id_level);
        virtual void dump_tree(int id_level); 
        ...
        ast_idlist* get_idlist() {return idlist;}
...
class ast_procdef : public ast_node {
    public:
        virtual ~ast_procdef() {
            ...
            for(it=in_args.begin(); it!=in_args.end(); it++) delete *it;
            for(it=out_args.begin(); it!=out_args.end(); it++) delete *it;
            delete_symtabs();
    ...
    public:
        ...
        std::list<ast_argdecl*>& get_in_args() {return in_args;}
        std::list<ast_argdecl*>& get_out_args() {return out_args;}
        ast_sentblock* get_body() {return sents;}
...
class gm_builtin_def; // defined in gm_builtin.h
...
class ast_expr : public ast_node 
{
    ...
    public: 
        static ast_expr* new_id_expr(ast_id* id)  {
            ast_expr* E = new ast_expr(); E->expr_class = GMEXPR_ID;
        ...
        static ast_expr* new_ival_expr(long ival){
            ...
            E->ival = ival; return E;}
        static ast_expr* new_fval_expr(double fval){
            ast_expr* E = new ast_expr(); E->expr_class = GMEXPR_FVAL; 
            E->type_of_expression = GMTYPE_FLOAT; // DOUBLE?
            E->fval = fval; return E;}
        ...
        static ast_expr* new_inf_expr(bool is_p){
            ...
            E->plus_inf = is_p; return E;}
        static ast_expr* new_typeconv_expr(int target_type, ast_expr* l){
            ast_expr* E = new ast_expr(); 
            E->expr_class = GMEXPR_UOP;  E->op_type = GMOP_TYPEC;
            E->type_of_expression = target_type; // GMTYPE_xxx
            E->left = l; l->up = E; l->parent = E;
            return E;}
        static ast_expr* new_uop_expr(int op_type, ast_expr* l){
            ast_expr* E = new ast_expr(); E->expr_class = GMEXPR_UOP;  E->op_type = op_type;
            E->type_of_expression = GMTYPE_UNKNOWN_NUMERIC;
            E->left = l; l->up = E; l->parent = E;
            return E;}
        static ast_expr* new_biop_expr(int op_type, ast_expr* l, ast_expr* r) {
            ast_expr* E = new ast_expr(); E->expr_class = GMEXPR_BIOP;  E->op_type = op_type;
            E->left = l; E->right = r; l->up = E; r->up = E; r->is_right = true; 
            E->type_of_expression = GMTYPE_UNKNOWN_NUMERIC;
            l->parent = r->parent = E;
            return E; }
        static ast_expr* new_luop_expr(int op_type, ast_expr* l) {
            ast_expr* E = new ast_expr(); E->expr_class = GMEXPR_LUOP;  E->op_type = op_type;
            E->type_of_expression = GMTYPE_BOOL;
            E->left = l; l->up = E; l->parent = E;
            return E;}
        static ast_expr* new_lbiop_expr(int op_type, ast_expr* l, ast_expr* r) {
            ast_expr* E = new ast_expr(); E->expr_class = GMEXPR_LBIOP;  E->op_type = op_type;
            E->left = l; E->right = r; l->up = E; r->up = E; r->is_right = true; 
            E->type_of_expression = GMTYPE_BOOL;
            l->parent = r->parent = E;
            return E; }
        static ast_expr* new_comp_expr(int op_type, ast_expr* l, ast_expr* r) {
            ast_expr* E = new ast_expr(); E->expr_class = GMEXPR_COMP;  E->op_type = op_type;
            E->left = l; E->right = r; l->up = E; r->up = E; r->is_right = true; 
            E->type_of_expression = GMTYPE_BOOL;
            l->parent = r->parent = E;
            return E; }
        ...
        static ast_expr* new_ternary_expr(ast_expr* cond, ast_expr* left, ast_expr* right) {
            ast_expr* E = new ast_expr(); E->expr_class = GMEXPR_TER; E->op_type = GMOP_TER;
            E->left = left; E->right = right; E->cond = cond;
    ...
    public:
        ...
        bool is_boolean_literal() {return expr_class == GMEXPR_BVAL;}
        bool is_inf() {return expr_class == GMEXPR_INF;}
        bool is_literal() {return is_int_literal() || is_float_literal() || is_boolean_literal() || is_inf() ;}
        ...
        bool is_plus_inf() {return is_inf() && plus_inf;} // true o
        ast_id* get_id() {return id1;}
        ast_field* get_field() {return field;}

        int get_opclass() {return expr_class;}
...
class ast_expr_reduce : public ast_expr 
{
    ...
    static ast_expr_reduce* 
        new_reduce_expr(int optype, 
            ast_id* iter, ast_id* src, int iter_op,
            ast_expr* body, ast_expr* filter = NULL) {
    ...
    int get_iter_type() {return iter_type;}
    void set_iter_type(int i) {iter_type = i;}
    int get_reduce_type() {return reduce_type;}
...
class ast_assign : public ast_sent {
    public:
        virtual ~ast_assign() {delete lhs_scala; delete lhs_field; delete rhs; delete bound;}
    ...
    public:
        static ast_assign* new_assign_scala(ast_id* id, ast_expr* r, int assign_type=GMASSIGN_NORMAL, ast_id* itor=NULL, int reduce_type=GMREDUCE_NULL){
            ...
            A->lhs_scala = id; A->rhs = r; 
            id->set_parent(A); r->set_parent(A);
        ...
        int get_assign_type() {return assign_type;}
        int get_lhs_type() {return lhs_type;}
        int get_reduce_type() {return reduce_type;}
        void set_assign_type(int a) {assign_type =  a;}
        void set_reduce_type(int a) {reduce_type =  a;}

        ast_id* get_lhs_scala() {return lhs_scala;}
        ast_field* get_lhs_field() {return lhs_field;}
        ast_expr* get_rhs() {return rhs;}
        ast_id* get_bound() {return bound;}
        void set_bound(ast_id* i) {bound = i; if (bound!=NULL) i->set_parent(this);}
        bool is_reduce_assign() { return assign_type== GMASSIGN_REDUCE;}
        bool is_defer_assign() { return assign_type== GMASSIGN_DEFER;}
...
class ast_vardecl : public ast_sent
{
    ...
    public:
        ...
        static ast_vardecl* new_vardecl(
                ast_typedecl* type,
                ast_idlist* id) {
        ...
        static ast_vardecl* new_vardecl(
                ...
                ast_id* id) {
            ast_vardecl* d = new ast_vardecl(); 
            ast_idlist* idl = new ast_idlist(); idl->add_id(id);
            d->idlist = idl; d->type = type;
            idl->set_parent(d); type->set_parent(d);
            return d;
        }
        static ast_vardecl* new_vardecl_init(
                ast_typedecl* type,
                ast_id* id,
                ast_expr* init) {
            ast_vardecl* d = new ast_vardecl(); 
            ast_idlist* idl = new ast_idlist(); idl->add_id(id);
            d->idlist = idl; d->type = type;d->init_expr = init;
        ...
        ast_expr* get_init() {return init_expr;}
        void set_init(ast_expr* v) {init_expr = v; if (v!=NULL) v->set_parent(this);}

        bool is_tc_finished() {return tc_finished;}
        void set_tc_finished(bool b) {tc_finished = b;} // who calls it?
...
class ast_return: public ast_sent {
    protected:
        ast_return():ast_sent(AST_RETURN),expr(NULL) {}
        ast_expr* expr;

    public:
        ~ast_return() {delete expr;}
...
class ast_foreach : public ast_sent
{
    ...
    public:
        // iterate on a graph
        static ast_foreach* new_foreach(
                ast_id* it, ast_id* src, ast_sent* b, 
                int iter_type, ast_expr* cond=NULL) 
        {
        ...
        ast_id* get_iterator() {return iterator;}
        ast_sent* get_body() {return body;}
        ast_expr* get_filter() {return cond;}
        ...
        bool is_sequential() { return seq_exe; } // sequential execution
        void set_sequential(bool b) {seq_exe = b;}
        bool is_parallel() { return !is_sequential(); } // sequential execution

        // for set iterator
        bool is_reverse_iteration() {return use_reverse;}
        void set_reverse_iteration(bool b) {use_reverse = b;}
...
private: 
    ast_call() : ast_sent(AST_CALL), b_in(NULL), is_blt_in(false) {}
...
class ast_if : public ast_sent
{
    ...
    private: 
        ast_if() : ast_sent(AST_IF), then_part(NULL), else_part(NULL), cond(NULL) {}
    ...
    public:
        ...
        ast_sent* get_else() {return else_part;}
...
class ast_while : public ast_sent
{
    ...
    private:
        ast_while() : ast_sent(AST_WHILE), body(NULL),cond(NULL),do_while(false) {}
...
{
    protected:
        ast_nop () : ast_sent(AST_NOP) {}
        ast_nop (int t) : ast_sent(AST_NOP) {set_subtype(t);}
    public:
        virtual ~ast_nop() {}
        int get_subtype() { return subtype;}
