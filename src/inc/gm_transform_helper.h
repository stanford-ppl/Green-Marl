...
bool gm_replace_symbol_access_scalar_field(ast_node* top, gm_symtab_entry* src, gm_symtab_entry* t_drv, gm_symtab_entry* target, bool chage_rhs = true,
        bool change_lhs = true);
bool gm_replace_symbol_access_field_scalar(ast_node* top, gm_symtab_entry* src_drv, gm_symtab_entry* src, gm_symtab_entry* target, bool chage_rhs = true,
        bool change_lhs = true);
bool gm_replace_symbol_access_field_field(ast_node* top, gm_symtab_entry* src_drv, gm_symtab_entry* src, gm_symtab_entry* t_drv, gm_symtab_entry* target,
        bool chage_rhs = true, bool change_lhs = true);
...
ast_foreach* gm_new_foreach_after_tc(ast_id* it, ast_id* src, ast_sent* body, int iter_type, ast_field* src_field=NULL);
...
typedef std::map<gm_rda_key, gm_rda_set> gm_rda_result;

// (Call to invoke analysis)
// R should be empty at the beginning, but filled after analysis
extern void gm_do_reaching_def_analysis(ast_procdef* p, gm_rda_result& R); 
...
extern bool gm_has_any_reaching_defs(gm_rda_result& R, ast_id* scalar);
extern gm_rda_set& gm_get_all_reaching_defs(gm_rda_result& R, ast_id* scalar);
