#ifndef GM_GIRAPH_BASICBLOCK_H
#define GM_GIRAPH_BASICBLOCK_H

#include "gm_ast.h"
#include "gm_traverse.h"
#include "giraph_syminfo.h"
#include "giraph_comminfo.h"
#include "gm_rw_analysis.h"
#include <list>

enum {
    GM_GIRAPH_BBTYPE_SEQ,
    GM_GIRAPH_BBTYPE_IF_COND,
    GM_GIRAPH_BBTYPE_WHILE_COND,
    GM_GIRAPH_BBTYPE_BEGIN_VERTEX,
    GM_GIRAPH_BBTYPE_PREPARE1,
    GM_GIRAPH_BBTYPE_PREPARE2,
    GM_GIRAPH_BBTYPE_MERGED_TAIL,
};

class gm_giraph_basic_block {
    public:
    gm_giraph_basic_block(int _id, int _type=GM_GIRAPH_BBTYPE_SEQ): id(_id), type(_type), after_vertex(false) /* ,_has_sender(false)*/ {
        for_info = ast_id::new_id("",0,0);
    }
    virtual ~gm_giraph_basic_block() {
        std::map<gm_symtab_entry*, giraph_syminfo*>::iterator I;
        for(I=symbols.begin(); I!=symbols.end();I++)
        {
            giraph_syminfo* s = I->second;
            delete s;
        }
    }
    ...
    gm_giraph_basic_block* get_nth_exit(int n) {return exits[n];}
    ...
    // while: body[0], exit[1]
    //-------------------------------
    void add_exit(gm_giraph_basic_block* b, bool add_reverse=true) {
        assert(b!=this);
        exits.push_back(b);
        if (add_reverse)
    ...
    void add_entry(gm_giraph_basic_block* b) {
        assert(b!=this);
        entries.push_back(b);
    }
    void update_entry_from(gm_giraph_basic_block* old, gm_giraph_basic_block* new_one)
    {
        assert(new_one!=this);
    ...
    void update_exit_to(gm_giraph_basic_block* old, gm_giraph_basic_block* new_one)
    {
        assert(new_one!=this);
    ...
    gm_giraph_basic_block* get_nth_entry(int n) {return entries[n];}
    ...
    bool is_vertex() {return (get_type() == GM_GIRAPH_BBTYPE_BEGIN_VERTEX);}
    //bool has_sender() {return _has_sender;}
    //void set_has_sender(bool b) {_has_sender = b;}
    bool is_prepare() {return (get_type() == GM_GIRAPH_BBTYPE_PREPARE1) ||
        (get_type() == GM_GIRAPH_BBTYPE_PREPARE2);}

    void add_nested_receiver(ast_foreach* fe)               {
        gm_giraph_comm_unit U(GIRAPH_COMM_NESTED, fe); add_receiver(U);
    }
    void add_random_write_receiver(ast_sentblock* sb, gm_symtab_entry* sym)               {
        gm_giraph_comm_unit U(GIRAPH_COMM_RANDOM_WRITE, sb, sym); add_receiver(U);
    }
    void add_receiver(gm_giraph_comm_unit& u)                  {receivers.push_back(u);}
    std::list<gm_giraph_comm_unit>&  get_receivers()           {return receivers;}
...
private:
    ...
    std::list<gm_giraph_comm_unit> receivers;

    std::vector<gm_giraph_basic_block*> exits;
    std::vector<gm_giraph_basic_block*> entries;
    int id;
    int type;  // GM_GIRAPH_BBTYPE_...
    bool after_vertex;
    //bool _has_sender;

    // map of used symbols inside this BB
    std::map<gm_symtab_entry*, giraph_syminfo*> symbols;

public:
    giraph_syminfo* find_symbol_info(gm_symtab_entry *sym) {
        if (symbols.find(sym) == symbols.end())
            return NULL;
        else 
            return symbols.find(sym)->second;
    }
    void add_symbol_info(gm_symtab_entry *sym,giraph_syminfo* info)
    {
        symbols[sym] = info;
    }

    std::map<gm_symtab_entry*, giraph_syminfo*>& get_symbols() {return symbols;}
...
public:
    ...
    void copy_info_from(gm_giraph_basic_block* bb) {for_info->copy_info_from(bb->for_info);}
};

class giraph_apply_bb {
public:
    virtual void apply(gm_giraph_basic_block* b)=0;
...
class giraph_apply_bb_ast : public gm_apply, public giraph_apply_bb
{
public:
    giraph_apply_bb_ast() : _curr(NULL), _under_receiver(false),_is_post(false), _is_pre(true), _receiver_type(GIRAPH_COMM_NESTED) {}

    // defined in gm_giraph_misc.cc
    virtual void apply(gm_giraph_basic_block* b);
    gm_giraph_basic_block *get_curr_BB() {return _curr;}
...
protected:
    gm_giraph_basic_block *_curr;
    bool _under_receiver;
    bool _is_post;
    bool _is_pre;
    int  _receiver_type;  // GIRAPH_COMM_NESTED, COMM_RAND_WRITE
...
bool giraph_bb_apply_until_no_change(gm_giraph_basic_block* entry, giraph_apply_bb* apply);
void giraph_bb_apply_only_once(gm_giraph_basic_block* entry, giraph_apply_bb* apply);  // in DFS order

void giraph_bb_print_all(gm_giraph_basic_block* entry);

// traverse BB (only once with DFS order) and apply to each AST
void giraph_bb_traverse_ast(gm_giraph_basic_block* entry,
                         giraph_apply_bb_ast* apply, bool is_post, bool is_pre);

// traverse single BB only
void giraph_bb_traverse_ast_single(gm_giraph_basic_block* entry,
                         giraph_apply_bb_ast* apply, bool is_post, bool is_pre);



gm_rwinfo_sets* gm_giraph_get_rwinfo_from_bb(gm_giraph_basic_block* BB, gm_rwinfo_sets* S);
gm_rwinfo_sets* gm_giraph_get_rwinfo_from_all_reachable_bb(gm_giraph_basic_block* BB, gm_rwinfo_sets* S);
#endif
