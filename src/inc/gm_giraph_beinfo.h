...
class gm_giraph_beinfo : public gm_backend_info
{
public:

    gm_giraph_beinfo(ast_procdef* d) {
        body = d;
        comm_id = 0;
        basicblock_id = 0;
    ...
    virtual ~gm_giraph_beinfo() {
    }

    void set_entry_basic_block(gm_giraph_basic_block* b) {bb_entry = b;}
    gm_giraph_basic_block* get_entry_basic_block() {return bb_entry;}
    std::list<gm_giraph_basic_block*>& get_basic_blocks() {return bb_blocks;}

public:
    //-------------------------------------------------------------------
    // inner loops (a.k.a. communication loops) are seperately managed.
    //-------------------------------------------------------------------
    std::set<gm_giraph_comm_unit, gm_giraph_comm_unit>&  get_communication_loops() {return comm_loops;}

    void add_communication_unit_nested(ast_foreach* fe) {
        gm_giraph_comm_unit U(GIRAPH_COMM_NESTED, fe);
    ...
    void add_communication_unit_random_write(ast_sentblock* sb, gm_symtab_entry* drv) {
        gm_giraph_comm_unit U(GIRAPH_COMM_RANDOM_WRITE, sb, drv);
        add_communication_unit(U);
    }
    
    gm_giraph_communication_symbol_info* find_communication_symbol_info(gm_giraph_comm_unit& U, gm_symtab_entry* sym);


    void add_communication_unit(gm_giraph_comm_unit& U);
    ...
    void add_communication_symbol(gm_giraph_comm_unit& U, gm_symtab_entry* sym);

    std::list<gm_giraph_communication_symbol_info>&
    ...
    std::list<gm_giraph_communication_symbol_info>&
        get_all_communication_symbols(gm_giraph_comm_unit& U);

    std::list<ast_sent*>& 
        get_random_write_sents(gm_giraph_comm_unit& U) {
    ...
    {
        gm_giraph_comm_unit U(GIRAPH_COMM_RANDOM_WRITE, sb, sym);
        return get_random_write_sents(U);
    }

    void add_random_write_sent(gm_giraph_comm_unit& U, ast_sent* s)
    {
    ...
    gm_giraph_communication_size_info*
    ...
    gm_giraph_communication_size_info*
        find_communication_size_info(gm_giraph_comm_unit& U);


    void compute_max_communication_size();
    gm_giraph_communication_size_info* get_max_communication_size() {return &max_comm_size;}
...
public:
    ...
    gm_giraph_congruent_msg_class* add_congruent_message_class(
            gm_giraph_communication_size_info* sz, gm_giraph_basic_block* bb);
    std::list<gm_giraph_congruent_msg_class*>& get_congruent_message_classes()  {return congruent_msg;}
...
private:
    ...
    gm_giraph_basic_block* bb_entry;               // entry for the procedure basic blocks (DAG)
    std::list<gm_giraph_basic_block*> bb_blocks;   // same as above DAG, but flattened as list
                                                // (created by gm_giraph_bb_find_reachable.cc)
    ...
    std::map<gm_giraph_comm_unit, std::list<gm_giraph_communication_symbol_info>, gm_giraph_comm_unit >   comm_symbol_info;
    std::map<gm_giraph_comm_unit, gm_giraph_communication_size_info*, gm_giraph_comm_unit >               comm_size_info;
    gm_giraph_communication_size_info                                         max_comm_size;

    // set of communications
    std::set<gm_giraph_comm_unit, gm_giraph_comm_unit> comm_loops;

    std::map<gm_giraph_comm_unit, std::list<ast_sent*>, gm_giraph_comm_unit>    random_write_sents;

    // congruent message class information
    std::list<gm_giraph_congruent_msg_class*>                                congruent_msg;
