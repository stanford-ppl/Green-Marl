...
void gm_flush_reproduce() {
    Out.flush();
}
void gm_newline_reproduce() {
    Out.NL();
}
void gm_redirect_reproduce(FILE *f) {
    Out.set_output_file(f);
}
void gm_baseindent_reproduce(int i) {
    Out.set_base_indent(i);
}
void gm_push_reproduce(char *s) {
    Out.push(s);
}
...
void ast_foreign::reproduce(int ind_level) {
    ...
    if (modified.size() > 0) {
        ...
        for (I = modified.begin(); I != modified.end(); I++, cnt++) {
            ...
            if (cnt != (int) (modified.size() - 1)) Out.push(", ");
        }
        Out.push("]");
    }
    Out.pushln(";");
