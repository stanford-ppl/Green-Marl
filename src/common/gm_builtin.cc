...
static int gm_get_type_from_string(const char* s) {
    ...
    if (gm_is_same_string(s, "Graph"))
        return GMTYPE_GRAPH;
    else if (gm_is_same_string(s, "Node"))
        return GMTYPE_NODE;
        ...
        return GMTYPE_LONG;
    else if (gm_is_same_string(s, "Float"))
        return GMTYPE_FLOAT;
    else if (gm_is_same_string(s, "Double"))
        return GMTYPE_DOUBLE;
    else if (gm_is_same_string(s, "N_S"))
        return GMTYPE_NSET;
    else if (gm_is_same_string(s, "E_S"))
        return GMTYPE_ESET;
    else if (gm_is_same_string(s, "N_O"))
        return GMTYPE_NORDER;
    else if (gm_is_same_string(s, "E_O"))
        return GMTYPE_EORDER;
    else if (gm_is_same_string(s, "N_Q"))
        return GMTYPE_NSEQ;
    else if (gm_is_same_string(s, "E_Q"))
        return GMTYPE_ESEQ;
    else if (gm_is_same_string(s, "Void"))
        return GMTYPE_VOID;
...
bool gm_builtin_def::has_info(const char* key) {
    std::string s(key);
    if (extra_info.find(s) == extra_info.end()) return false;
    return true;
}
...
gm_builtin_def* gm_builtin_manager::find_builtin_def(int source_type, const char* orgname) {
    ...
    for (i = defs.begin(); i != defs.end(); i++) {
        ...
        if (gm_is_same_string(orgname, d->get_orgname())) {
            ...
            if (def_src == GMTYPE_VOID) continue;assert(!gm_is_prim_type(def_src));
