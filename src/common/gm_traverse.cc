...
void ast_procdef::traverse(gm_apply*a, bool is_post, bool is_pre) {
    ...
    if (is_pre) {
        if (for_symtab) apply_symtabs(a, PRE_APPLY);
        if (for_id) apply_id(a, PRE_APPLY);
        if (for_proc) a->apply(this);
    }
    ...
    if (is_post) {
        if (for_symtab) apply_symtabs(a, POST_APPLY);
        if (for_id) apply_id(a, POST_APPLY);
        if (for_proc) {
...
void ast_sent::traverse(gm_apply*a, bool is_post, bool is_pre) {
    ...
    if (is_pre) {
        if (for_sent) a->apply(this);
        if (has_symtab() && for_symtab) apply_symtabs(a, PRE_APPLY);
    }
...
void ast_vardecl::traverse_sent(gm_apply*a, bool is_post, bool is_pre) {
    ...
    if (for_id) {
        ...
        if (is_pre) idl->apply_id(a, PRE_APPLY);

        if (is_post) idl->apply_id(a, POST_APPLY);
    }
}
...
void ast_foreach::traverse_sent(gm_apply*a, bool is_post, bool is_pre) {
    ...
    if (!a->is_traverse_local_expr_only()) ss->traverse(a, is_post, is_pre);
...
void ast_return::traverse_sent(gm_apply*a, bool is_post, bool is_pre) {
    if (get_expr() != NULL) get_expr()->traverse(a, is_post, is_pre);
}
