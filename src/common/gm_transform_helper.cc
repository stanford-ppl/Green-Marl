...
public:
    ...
    void set_scope_to_remove(gm_scope* s) {
        set_old_scope(s);
    }
};
...
public:
    gm_scope O;
    putnew_upper_scope_on_null() {
        set_for_symtab(true);
        O.push_symtabs(NULL, NULL, NULL);
        set_old_scope(&O);
    }
    void set_scope_to_put(gm_scope* s) {
        set_new_scope(s);
    }
};
...
static void gm_make_it_belong_to_sentblock_main(ast_sent* s, bool allow_nesting, bool need_fix_symtab) {
    ...
    if ((up->get_nodetype() == AST_SENTBLOCK) && (!allow_nesting)) return;
...
void gm_insert_sent_body_begin(ast_foreach* fe, ast_sent* target, bool need_fix_symtab) {
    ...
    if (s->get_nodetype() == AST_SENTBLOCK) {
        ...
        gm_add_sent_begin(s, target, need_fix_symtab);
    }
}
...
{
public:
    replace_subexpr_A(ast_expr* e_old, ast_expr* e_new) :
            gm_apply() {
        found = false;
        o = e_old;
        n = e_new;
...
{
public:
    gm_replace_traverse_t(gm_expr_replacement_t* E) :
            _CHECK(E) {
        set_for_sent(true);
        set_for_expr(true);
        _changed = false;
