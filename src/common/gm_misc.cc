...
const char* gm_get_op_string(int op_type) {
    const char* opstr = (op_type == GMOP_MULT) ? "*" : (op_type == GMOP_DIV) ? "/" : (op_type == GMOP_SUB) ? "-" : (op_type == GMOP_MOD) ? "%" :
...
const char* gm_get_reduce_string(int rop_type) {
    const char* opstr = (rop_type == GMREDUCE_PLUS) ? "+=" : (rop_type == GMREDUCE_MULT) ? "*=" : (rop_type == GMREDUCE_MIN) ? "min=" :
                        (rop_type == GMREDUCE_MAX) ? "max=" : (rop_type == GMREDUCE_AND) ? "&=" : (rop_type == GMREDUCE_OR) ? "|=" :
...
bool gm_need_paranthesis(int this_op, int up_op, bool is_right) {
    if (up_op == GMOP_TER) {
        ...
        if ((this_op == GMOP_TER) && is_right)
            return false;
        else
            return true;
    } else {
        if (gm_get_op_pred(this_op) > gm_get_op_pred(up_op))
            return true;

        else if (gm_get_op_pred(this_op) == gm_get_op_pred(up_op) && is_right)
            return true;
