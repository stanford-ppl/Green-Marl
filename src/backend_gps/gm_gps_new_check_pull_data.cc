...
public:
    ...
    bool apply(ast_sent* s) {
        ...
        else if (s->get_nodetype() == AST_CALL) {assert(false);}
        else if (s->get_nodetype() == AST_FOREIGN) { assert(false); 
...
private:
    ...
    int get_scope_from_driver(gm_symtab_entry* e, bool is_rarrow = false) 
    {
        ...
        else if (e->find_info_bool(GPS_FLAG_IS_EDGE_ITERATOR)) return GPS_NEW_SCOPE_EDGE;
