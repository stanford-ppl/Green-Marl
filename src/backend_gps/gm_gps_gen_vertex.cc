...
void gm_gps_gen::do_generate_vertex_property_class(bool is_edge_prop) {
    ...
    assert(proc != NULL);
    sprintf(temp, "public static class %s extends MinaWritable {", is_edge_prop ? "EdgeData" : "VertexData");
    Body.pushln(temp);
    ...
    if (FE.get_current_proc_info()->find_info_bool(GPS_FLAG_USE_REVERSE_EDGE)) {
        sprintf(temp, "%s [] %s; //reverse edges (node IDs) {should this to be marshalled?}", PREGEL_BE->get_lib()->is_node_type_int() ? "int" : "long",
                GPS_REV_NODE_ID);
        Body.pushln(temp);
    }
...
void gm_gps_gen::do_generate_message_class() {
    ...
    assert(proc != NULL);
    gm_gps_beinfo * info = (gm_gps_beinfo *) FE.get_current_backend_info();
    sprintf(temp, "public static class MessageData extends MinaWritable {");
    Body.pushln(temp);
...
void gm_gps_gen::do_generate_vertex_class() {
    ...
    if (FE.get_current_proc()->find_info_bool(GPS_FLAG_USE_EDGE_PROP)) {
        sprintf(temp, "extends Vertex< %s.VertexData, %s.EdgeData, %s.MessageData > {", proc_name, proc_name, proc_name);
    } else {
        sprintf(temp, "extends NullEdgeVertex< %s.VertexData, %s.MessageData > {", proc_name, proc_name);
    }
    Body.pushln(temp);
    Body.pop_indent();
    ...
    Body.push_indent();
    if (FE.get_current_proc()->find_info_bool(GPS_FLAG_USE_EDGE_PROP)) {
        sprintf(temp, "extends VertexFactory< %s.VertexData, %s.EdgeData, %s.MessageData > {", proc_name, proc_name, proc_name);
    } else {
        sprintf(temp, "extends NullEdgeVertexFactory< %s.VertexData, %s.MessageData > {", proc_name, proc_name);
    }
    Body.pushln(temp);
    Body.pop_indent();
    Body.pushln("@Override");
    if (FE.get_current_proc()->find_info_bool(GPS_FLAG_USE_EDGE_PROP)) {
        sprintf(temp, "public Vertex< %s.VertexData, %s.EdgeData, %s.MessageData > newInstance(CommandLine line) {", proc_name, proc_name, proc_name);
    } else {
        sprintf(temp, "public NullEdgeVertex< %s.VertexData, %s.MessageData > newInstance(CommandLine line) {", proc_name, proc_name);
    }
    Body.pushln(temp);
...
void gm_gps_gen::do_generate_vertex_state_body(gm_gps_basic_block *b) {
    ...
    char temp[1024];
    sprintf(temp, "private void _vertex_state_%d(Iterable<%s.MessageData> _msgs) {", id, FE.get_current_proc()->get_procname()->get_genname());
    Body.pushln(temp);
