...
public:
    ...
    virtual bool apply(ast_sent *s) {
        if (s->get_nodetype() == AST_BFS) {
            gm_backend_error(GM_ERROR_GPS_UNSUPPORTED_OP, s->get_line(), s->get_col(), "BFS or DFS");
            _error = true;
        }

        if (s->get_nodetype() == AST_RETURN) {
            if (foreach_depth > 0) {
                gm_backend_error(GM_ERROR_GPS_UNSUPPORTED_OP, s->get_line(), s->get_col(), "Return inside foreach");
                _error = true;
            }
        }
    ...
    virtual bool apply(gm_symtab_entry* e, int symtab_type) {
        ...
        if (gm_is_collection_type(type_id)) {
            ...
            if (_graph_defined) {
                gm_backend_error(GM_ERROR_GPS_MULTIPLE_GRAPH, e->getId()->get_line(), e->getId()->get_col(), e->getId()->get_orgname());
                _error = true;
            }
            _graph_defined = true;
        }
...
void gm_gps_opt_check_synthesizable::process(ast_procdef* proc) {
    ...
    if (!T.is_graph_defined()) {
        gm_backend_error(GM_ERROR_GPS_NO_GRAPH, proc->get_procname()->get_line(), proc->get_procname()->get_col());
    ...
    if (T2.is_rand_used()) beinfo->set_rand_used(true);
}
