...
extern void gm_giraph_find_double_nested_loops(ast_node* p, std::map<ast_foreach*, ast_foreach*>& MAP);
...
class giraph_opt_find_scalar_replace_target_t : public gm_apply
{

public:
    giraph_opt_find_scalar_replace_target_t(std::map<ast_foreach*, ast_foreach*>& M) : MAP(M)
    {
       set_for_sent(true);
       set_for_symtab(true);
...
void gm_giraph_opt_insert_temp_property::process(ast_procdef* p)
{
    //-------------------------------------
    // Find nested loops
    //-------------------------------------
    std::map<ast_foreach*, ast_foreach*> MAP;
    gm_giraph_find_double_nested_loops(p, MAP);

    //-------------------------------------
    // Find scalar targets
    //-------------------------------------
    giraph_opt_find_scalar_replace_target_t T(MAP);
    p->traverse_both(&T);
