...
class giraph_check_reverse_edge_t : public gm_apply {
public:
    giraph_check_reverse_edge_t() {
        set_for_sent(true);
        set_for_expr(true);
        r_edge = false;
        r_degree = false;
...
void gm_giraph_opt_check_reverse_edges::process(ast_procdef* p)
{
    giraph_check_reverse_edge_t T;
    p->traverse_pre(&T);
    if (T.use_rev_edge()) {
        FE.get_proc_info(p)->add_info_bool(GIRAPH_FLAG_USE_REVERSE_EDGE, true);
        // a special basic block will be added in create ebb state.
    }
    else if (T.use_in_degree()) {
        FE.get_proc_info(p)->add_info_bool(GIRAPH_FLAG_USE_IN_DEGREE, true);

        // define a new node_property for in_degree counting
        char* tmp_name = FE.voca_temp_name_and_add("_in_degree", NULL, true);
        FE.get_proc_info(p)->add_info_string(GIRAPH_NAME_IN_DEGREE_PROP, tmp_name);
