...
class giraph_new_check_depth_two_t : public gm_apply {
public:
    giraph_new_check_depth_two_t()
    {
        _error = false;
        foreach_depth = 0;
    ...
    virtual bool apply(ast_sent* s)
    {
        ...
        if (foreach_depth == 1) {
            ...
            {
                gm_backend_error(GM_ERROR_GIRAPH_UNSUPPORTED_RANGE_MASTER,
                         s->get_line(), s->get_col(), "");
                _error = true;
            }

            if (fe->is_sequential()) {
                gm_backend_error(GM_ERROR_GIRAPH_NEED_PARALLEL,
                         s->get_line(), s->get_col(), "");
                _error = true;
            }
        }
        ...
        else if (foreach_depth == 2) 
        {
                ...
                {
                    gm_backend_error(GM_ERROR_GIRAPH_UNSUPPORTED_RANGE_VERTEX,
                             s->get_line(), s->get_col(), "");
                    _error = true;
                }
                if (fe->is_sequential()) {
            ...
            _error = true;
            gm_backend_error(GM_ERROR_GIRAPH_NBR_LOOP_TOO_DEEP,
                         s->get_line(), s->get_col(), "");
        }
...
extern void gm_giraph_do_new_scope_analysis(ast_procdef* proc);
void gm_giraph_new_check_depth_two::process(ast_procdef* proc)
{
    // Check number of procedure name is same to the filename
    const char *fname = GIRAPH_BE.getFileName();
    assert(fname!=NULL);
    if (strcmp(proc->get_procname()->get_genname(), fname) != 0) 
    {
        gm_backend_error(GM_ERROR_GIRAPH_PROC_NAME,proc->get_procname()->get_genname(), fname);
        set_okay(false);
        return;
    }

    // analyze_symbol_scope should be done before.
    giraph_new_check_depth_two_t T;
    ...
    //-----------------------------------------
    if (!T.is_error())
        gm_giraph_do_new_scope_analysis(proc);
