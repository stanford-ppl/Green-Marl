...
class giraph_opt_simplify_outer_builtin_t : public gm_apply
{
public:
    giraph_opt_simplify_outer_builtin_t() {
       set_for_sent(true);
...
class giraph_opt_check_contain_builtin_through_t : public gm_apply
{
public:
    giraph_opt_check_contain_builtin_through_t(gm_symtab_entry* drv) {
       set_for_expr(true); 
...
static bool contains_built_in_through_driver(ast_sent* s, gm_symtab_entry* e)
{
     giraph_opt_check_contain_builtin_through_t T(e);
     s->traverse_pre(&T);
...
class giraph_opt_replace_builtin_t : public gm_apply
{
public:
    giraph_opt_replace_builtin_t(gm_symtab_entry* drv, ast_sentblock* scope,
        ...
        )
{
    ...
    giraph_opt_replace_builtin_t T(e, scope, already_defined_map);
    s->traverse_post(&T);
...
void gm_giraph_opt_simplify_expr1::process(ast_procdef* p)
{
    giraph_opt_simplify_outer_builtin_t T;
    p->traverse_both(&T);
    T.post_process();
}
