...
void gm_giraph_beinfo::add_communication_unit(gm_giraph_comm_unit& C)
{
    ...
    std::list<gm_giraph_communication_symbol_info> new_list;
    comm_symbol_info[C] = new_list;   // create empty list by copying.

    gm_giraph_communication_size_info *S = new gm_giraph_communication_size_info();
    S->id = issue_comm_id();
    comm_size_info[C] = S;  // create zero-sized communication 

}

gm_giraph_communication_symbol_info* gm_giraph_beinfo::find_communication_symbol_info(gm_giraph_comm_unit& C, gm_symtab_entry* sym)
{
    assert(comm_symbol_info.find(C) != comm_symbol_info.end());
    std::list<gm_giraph_communication_symbol_info>& sym_info = comm_symbol_info[C];
    std::list<gm_giraph_communication_symbol_info>::iterator I;
    for(I=sym_info.begin(); I!= sym_info.end();I++)
    {
        gm_giraph_communication_symbol_info& S = *I;
...
void gm_giraph_beinfo::add_communication_symbol(gm_giraph_comm_unit& C, gm_symtab_entry* sym)
{
    assert(comm_symbol_info.find(C) != comm_symbol_info.end());

    std::list<gm_giraph_communication_symbol_info>& sym_info = comm_symbol_info[C];
    std::list<gm_giraph_communication_symbol_info>::iterator I;
    gm_giraph_communication_size_info* size_info = comm_size_info[C];
    ...
    if (sym->getType()->is_property()) {
        ...
        if (GIRAPH_BE.get_lib()->is_node_type_int())
    ...
    gm_giraph_communication_symbol_info II;
    II.symbol = sym;
...
std::list<gm_giraph_communication_symbol_info>& gm_giraph_beinfo::get_all_communication_symbols(
...
/*
gm_giraph_communication_symbol_info& gm_giraph_beinfo::find_communication_symbol_info(
        ast_foreach* fe, int comm_type, gm_symtab_entry* sym)
{
    gm_giraph_comm_unit U(comm_type, fe);
    assert(comm_symbol_info.find(U) != comm_symbol_info.end());

    std::list<gm_giraph_communication_symbol_info>& sym_info = comm_symbol_info[U];
    std::list<gm_giraph_communication_symbol_info>::iterator I;
    for(I=sym_info.begin(); I!= sym_info.end();I++)
    {
        gm_giraph_communication_symbol_info& S = *I;
...
gm_giraph_communication_size_info* gm_giraph_beinfo::find_communication_size_info(
        gm_giraph_comm_unit& U)
{
...
void gm_giraph_beinfo::compute_max_communication_size()
{
    std::set<gm_giraph_comm_unit, gm_giraph_comm_unit>::iterator I;
    for(I = this->comm_loops.begin(); I!= this->comm_loops.end(); I++)
    {
        ast_foreach* fe = I->fe;
        gm_giraph_communication_size_info& size_info = *(comm_size_info[*I]);
...
gm_giraph_congruent_msg_class* 
    gm_giraph_beinfo::add_congruent_message_class(
        gm_giraph_communication_size_info* sz,
        gm_giraph_basic_block* bb)
{
    gm_giraph_congruent_msg_class *C = 
        new gm_giraph_congruent_msg_class();
