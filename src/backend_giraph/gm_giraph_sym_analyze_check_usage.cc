...
//---------------------------------------------------------------------
class giraph_merge_symbol_usage_t : public giraph_apply_bb_ast  
{
    static bool const IS_SCALAR;
    static bool const IS_FIELD;

    public:
        giraph_merge_symbol_usage_t(gm_giraph_beinfo* i) {
            set_for_sent(true);
            set_for_expr(true);
    ...
    virtual bool apply(ast_sent* s) 
    {
        ...
        if (s->get_nodetype() == AST_ASSIGN)
        {
            ...
            random_write_target_sb = (ast_sentblock*) 
                s->find_info_ptr(GIRAPH_FLAG_SENT_BLOCK_FOR_RANDOM_WRITE_ASSIGN);
            ...
            if (foreach_depth > 1) {
                if (context == GIRAPH_CONTEXT_MASTER) // inner loop
                    return true;
            }
            ...
            int lhs_reduce = a->is_reduce_assign() ? GIRAPH_SYM_USED_AS_REDUCE : GIRAPH_SYM_USED_AS_LHS;
    ...
    virtual bool apply(ast_expr* e) 
    {
        ...
        //----------------------------------------------------------
        int context = get_current_context();
        int used_type = GIRAPH_SYM_USED_AS_RHS;
        bool is_id = e->is_id();
        ...
        if (is_random_write_target) {
            if (is_id) {
                giraph_syminfo* syminfo = giraph_get_global_syminfo(tg);
        ...
        else if (context == GIRAPH_CONTEXT_VERTEX) {
            if (target_is_edge) { 
                comm_symbol = false; 
                ...
                if (is_id) { 
                    ...
                    if ((drv == out_iterator) || (drv->find_info_bool(GIRAPH_FLAG_EDGE_DEFINED_INNER)))
                        comm_symbol = true;
                    else
                        ignored_symbol = true; // local to the receiver
                }
        ...
        if (comm_symbol) {
            ...
            tg->getSymInfo()->add_info_bool(GIRAPH_FLAG_COMM_SYMBOL, true);
        }
        return true;
    }
    ...
    protected:
        ...
        int get_current_context() {
            ...
            if (!get_curr_BB()->is_vertex()) {
                // master context
                context = GIRAPH_CONTEXT_MASTER;
            }
            else {
                // sender/recevier
                if (is_under_receiver_traverse()) 
                    context = GIRAPH_CONTEXT_RECEIVER;
                else
                    context = GIRAPH_CONTEXT_VERTEX;
            }
                ...
                bool is_scalar, int usage, int context, int r_type = GMREDUCE_NULL)
        {
            ...
            printf("Add usage : %s for BB : %d, context: %s\n", 
                    i->get_genname(),
                    get_curr_BB()->get_id(), 
                    (context == GIRAPH_CONTEXT_MASTER) ? "master" :
                    (context == GIRAPH_CONTEXT_RECEIVER) ? "receiver" : "vertex"
                    );
                    */
        }

        giraph_syminfo* get_or_create_global_syminfo(ast_id *i, bool is_scalar)
        {
            gm_symtab_entry* sym = i->getSymInfo(); 

            ast_extra_info* info = sym->find_info(GIRAPH_TAG_BB_USAGE);
            giraph_syminfo* syminfo;
            if (info == NULL)  {
                syminfo = new giraph_syminfo(is_scalar);
                sym->add_info(GIRAPH_TAG_BB_USAGE, syminfo);
            } else {
                syminfo = (giraph_syminfo*) info;
            }
            return syminfo;
        }

        giraph_syminfo* get_or_create_local_syminfo(ast_id *i, bool is_scalar)
        {
            gm_symtab_entry* sym = i->getSymInfo(); 
...
const bool giraph_merge_symbol_usage_t::IS_SCALAR = true;
const bool giraph_merge_symbol_usage_t::IS_FIELD = false;


void gm_giraph_opt_analyze_symbol_usage::process(ast_procdef* p)
{
    ...
    // traverse BB
    giraph_merge_symbol_usage_t T(beinfo);
    giraph_bb_traverse_ast(entry_BB, &T, true, true);
