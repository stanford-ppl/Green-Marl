...
//-------------------------------------------------------
class gm_giraph_find_rwinfo_simple : public giraph_apply_bb_ast 
{
public:
    // find
    gm_giraph_find_rwinfo_simple( gm_rwinfo_sets* _SS) : S(_SS) {
        set_for_rhs(true);
        set_for_lhs(true);
        set_for_sent(true);
    ...
    virtual bool apply(ast_sent* s) 
    {
        ...
        {
            if (s->find_info_bool(GIRAPH_FLAG_IS_OUTER_LOOP)) {
                outer_loop = (ast_foreach*) s;
            }
            else if (s->find_info_bool(GIRAPH_FLAG_IS_INNER_LOOP)) {
    ...
    virtual bool apply_lhs(ast_id* id) {
        ...
        //    }
        //-----------------------------------------------------
        giraph_syminfo* syminfo = giraph_get_global_syminfo(id);
        if (syminfo == NULL) {
            return true;
        }
        if (is_inner_loop()) {
    ...
    virtual bool apply_rhs(ast_field* field) {
        ...
        if (is_inner_loop()) {
            ...
            else if (drv->find_info_bool(GIRAPH_FLAG_EDGE_DEFINED_INNER)){
                if (is_under_receiver_traverse()) return true;
            }
            else {
...
gm_rwinfo_sets* gm_giraph_get_rwinfo_from_bb(giraph_bb* BB, gm_rwinfo_sets* S)
{
    if (S == NULL)
    ...
    //-------------------------------------------
    gm_giraph_find_rwinfo_simple T(S);
    giraph_bb_traverse_ast_single(BB, &T, true, true);  // post && pre

    return S;
}

gm_rwinfo_sets* gm_giraph_get_rwinfo_from_all_reachable_bb(giraph_bb* BB, gm_rwinfo_sets* S)
{
    ...
    //-------------------------------------------
    gm_giraph_find_rwinfo_simple T(S);
    giraph_bb_traverse_ast(BB, &T, true, true);  // post && pre
