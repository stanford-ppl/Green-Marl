        ...
        gm_symtab_entry* e, int op)
{
    ...
    int* curr_state = (int*)
        fe->find_info_map_value(GIRAPH_MAP_EDGE_PROP_ACCESS, e);

    // first access
    if (curr_state == NULL) {
        int* new_state = new int;
        *new_state = (op == SENDING)? GIRAPH_ENUM_EDGE_VALUE_SENT:
                     GIRAPH_ENUM_EDGE_VALUE_WRITE;

        fe->add_info_map_key_value(GIRAPH_MAP_EDGE_PROP_ACCESS, e, new_state);
    }
    else {
        int curr_state_val = *curr_state;
        switch(curr_state_val) {
            case GIRAPH_ENUM_EDGE_VALUE_ERROR: //already error
                return false;

            case GIRAPH_ENUM_EDGE_VALUE_WRITE:
                if (op == SENDING) 
                   *curr_state = GIRAPH_ENUM_EDGE_VALUE_WRITE_SENT; 
                return false; // no error
        
            case GIRAPH_ENUM_EDGE_VALUE_SENT:
                if (op == WRITING) 
                   *curr_state = GIRAPH_ENUM_EDGE_VALUE_SENT_WRITE;
                return false; // no error

            case GIRAPH_ENUM_EDGE_VALUE_WRITE_SENT:
                if (op == WRITING)
                   *curr_state = GIRAPH_ENUM_EDGE_VALUE_SENT_WRITE;
                return false;

            case GIRAPH_ENUM_EDGE_VALUE_SENT_WRITE:
                if (op == SENDING) { // sending two versions!
                   *curr_state = GIRAPH_ENUM_EDGE_VALUE_ERROR;
                   return true; // ERROR
                }
                else
                    return false;
...
class giraph_check_edge_value_t : public gm_apply {
public:
    giraph_check_edge_value_t()  {
        foreach_depth = 0;
    ...
    virtual bool apply(gm_symtab_entry *e, int type)
    {
        ...
        {
            e->add_info_bool(GIRAPH_FLAG_EDGE_DEFINED_INNER, true);
            inner_loop->add_info_bool(GIRAPH_FLAG_EDGE_DEFINING_INNER, true);
        }
        return true;
    }
            ...
            if (!a->is_target_scalar()) {
                ...
                if (sym->getType()->is_edge_compatible()) {
                    ...
                    if (sym->find_info_bool(GIRAPH_FLAG_EDGE_DEFINED_INNER)) {
                        ...
                        if (conditional) {
                            gm_backend_error(GM_ERROR_GIRAPH_EDGE_WRITE_CONDITIONAL, 
                        ...
                        inner_loop->add_info_list_element(GIRAPH_LIST_EDGE_PROP_WRITE,s);
                        ...
                        gm_backend_error(GM_ERROR_GIRAPH_EDGE_WRITE_RANDOM, 
                            a->get_lhs_field()->get_line(),
                            a->get_lhs_field()->get_col());
                        set_error(true);
                    }
                ...
                if (sym->getType()->is_edge()) {
                    if (sym->find_info_bool(GIRAPH_FLAG_EDGE_DEFINED_INNER)) {
                        ...
                        if (rhs->is_builtin()) {
                            ...
                            {
                                error = false;
                                a->add_info_bool(GIRAPH_FLAG_EDGE_DEFINING_WRITE, true);
                            }
                        }
    ...
    virtual bool apply(ast_expr* e)
    {
        ...
        if (f->getSourceTypeInfo()->is_edge_compatible()) {
            ...
            if (!f->get_first()->getSymInfo()->find_info_bool(GIRAPH_FLAG_EDGE_DEFINED_INNER)) {
                gm_backend_error(GM_ERROR_GIRAPH_EDGE_READ_RANDOM, 
        ...
        if (target_is_edge) { 
            if (f->getSourceTypeInfo()->is_node_compatible()) {
                if (f->get_first()->getSymInfo() == inner_iter) {
                    gm_backend_error(GM_ERROR_GIRAPH_EDGE_WRITE_RHS, 
                            f->get_line(), f->get_col(), 
            ...
            if (f->getSourceTypeInfo()->is_edge_compatible()) {
                ...
                if (b) {
                    gm_backend_error(GM_ERROR_GIRAPH_EDGE_SEND_VERSIONS, 
                            f->get_line(), f->get_col(), 
                            f->get_first()->get_orgname());
                    set_error(true);
                }
...
void gm_giraph_opt_check_edge_value::process(ast_procdef* proc)
{
    giraph_check_edge_value_t T;
