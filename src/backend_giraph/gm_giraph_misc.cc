...
void gm_giraph_basic_block::reproduce_sents()
{
    if(type==GM_GIRAPH_BBTYPE_IF_COND) {
        prepare_iter();
        ...
        gm_newline_reproduce(); 
        gm_flush_reproduce(); 
    } else if ( (type == GM_GIRAPH_BBTYPE_BEGIN_VERTEX) || (type == GM_GIRAPH_BBTYPE_SEQ)) {

        if ((type == GM_GIRAPH_BBTYPE_BEGIN_VERTEX) && (has_receiver())) {
            gm_giraph_beinfo* info = (gm_giraph_beinfo*) FE.get_current_backend_info();
            std::list<gm_giraph_comm_unit>& L =  get_receivers(); 
            std::list<gm_giraph_comm_unit>::iterator I;
            for(I=L.begin();I!=L.end();I++)
            {
                gm_giraph_comm_unit U = *I;
                if (U.type_of_comm == GIRAPH_COMM_NESTED) {
                    gm_push_reproduce((char*)"//Receive Nested Loop"); 
                    gm_newline_reproduce(); 
                    ast_foreach* fe = U.fe;
                    fe->reproduce(0);
                }
                else if (U.type_of_comm == GIRAPH_COMM_RANDOM_WRITE) 
                {
        ...
        { 
            s->reproduce(0);
            s = get_next();
            if ((type == GM_GIRAPH_BBTYPE_BEGIN_VERTEX) && (s!=NULL))
                gm_newline_reproduce();
        }
        gm_flush_reproduce(); 
   } else if ((type == GM_GIRAPH_BBTYPE_PREPARE1) || (type == GM_GIRAPH_BBTYPE_PREPARE2) || (type == GM_GIRAPH_BBTYPE_MERGED_TAIL)) {
       // do nothing;
   } else {
       assert(false);
   }
...
void gm_giraph_basic_block::print()
{
    printf("[--------------------\n");
    printf("%d (%s):\n", id,
            (type==GM_GIRAPH_BBTYPE_SEQ) ?        "SEQ" :
            (type==GM_GIRAPH_BBTYPE_IF_COND) ?    "IF" :
            (type==GM_GIRAPH_BBTYPE_WHILE_COND) ? "WHILE" :
            (type==GM_GIRAPH_BBTYPE_BEGIN_VERTEX) ? "PAR" : "???"
            );
...
// depth-first recurse
static void bb_apply_recurse(std::set<gm_giraph_basic_block*>& set, gm_giraph_basic_block* B, giraph_apply_bb* apply)
{
    apply->apply(B);
    set.insert(B);
    for(int i=0;i <B->get_num_exits(); i++)
    {
        gm_giraph_basic_block* b = B->get_nth_exit(i);
...
void giraph_bb_apply_only_once(gm_giraph_basic_block* entry, giraph_apply_bb* apply)
{
    std::set<gm_giraph_basic_block*> set;
    set.clear();
...
class giraph_print_apply : public giraph_apply_bb {
    public:
...
void giraph_bb_print_all(gm_giraph_basic_block* entry)  // return or of has_changed
{
     giraph_print_apply G;
     giraph_bb_apply_only_once(entry, &G);
}
...
void giraph_apply_bb_ast::apply(gm_giraph_basic_block* b)
{
    _curr = b;
    int type = _curr->get_type();
    //printf("visiting :%d\n", _curr->get_id());
    if (type == GM_GIRAPH_BBTYPE_SEQ) 
    {
    ...
    else if (type==GM_GIRAPH_BBTYPE_BEGIN_VERTEX)
    {

        // traverse receiver
        if (_curr->has_receiver())
        {
            std::list<gm_giraph_comm_unit>& R = _curr->get_receivers();
            std::list<gm_giraph_comm_unit>::iterator I;
            ...
            for(I=R.begin(); I!=R.end(); I++)
            {
                ...
                } else if (U.get_type() == GIRAPH_COMM_RANDOM_WRITE) {
                    gm_giraph_beinfo* info = (gm_giraph_beinfo*) FE.get_current_backend_info();
                    std::list<ast_sent*>& LL = info->get_random_write_sents(U);
                    std::list<ast_sent*>::iterator II;
                    for(II=LL.begin(); II!=LL.end(); II++) 
    ...
    else if (type == GM_GIRAPH_BBTYPE_PREPARE1) {
        // nothing

    } else if (type == GM_GIRAPH_BBTYPE_PREPARE2) {
        // nothing
    }
                         ...
                         bool is_pre)
{
    ...
    // apply it once
    giraph_bb_apply_only_once(entry, apply);
