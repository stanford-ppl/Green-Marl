...
void gm_giraphlib::generate_broadcast_state_master(
...
void gm_giraphlib::generate_receive_state_vertex(
    const char* state_var, gm_code_writer& Body)
{
    char temp[1024];
...
void gm_giraphlib::generate_receive_isFirst_vertex(
    const char* is_first_var, gm_code_writer& Body)
{
    char temp[1024];
...
void gm_giraphlib::generate_parameter_read_vertex(ast_id* id, gm_code_writer& Body)
{
    ...
    {
        case GMTYPE_BOOL:   sprintf(temp, "getBoolean(\"%s\", false)", id->get_genname()); break;
        case GMTYPE_INT:    sprintf(temp, "getInt(\"%s\", -1)", id->get_genname()); break;
        case GMTYPE_LONG:   sprintf(temp, "getLong(\"%s\", -1L)", id->get_genname()); break;
        case GMTYPE_FLOAT:  sprintf(temp, "getFloat(\"%s\", -1.0f)", id->get_genname()); break;
        ...
        case GMTYPE_DOUBLE: sprintf(temp, "getFloat(\"%s\", -1.0f)", id->get_genname()); break;
        case GMTYPE_NODE:   is_node_type_int() ?
...
void gm_giraphlib::generate_broadcast_variable_type(
...
static void genPutIOB(const char* name, int gm_type, gm_code_writer& Body, gm_giraphlib* lib)
{
    ...
    Body.push("out.");
    switch(gm_type) {
        case GMTYPE_INT:    Body.push("writeInt"); break;
        case GMTYPE_LONG:   Body.push("writeLong"); break;
        case GMTYPE_FLOAT:  Body.push("writeFloat"); break;
        case GMTYPE_DOUBLE: Body.push("writeDouble"); break;
        case GMTYPE_BOOL:   Body.push("writeBoolean"); break;
        case GMTYPE_NODE:
            if (lib->is_node_type_int()) {Body.push("writeInt"); break;}
            else                         {Body.push("writeLong"); break;}
        case GMTYPE_EDGE:
            if (lib->is_edge_type_int()) {Body.push("writeInt"); break;}
            else                         {Body.push("writeLong"); break;}
        default: assert(false); break;
    }
    Body.push("(");
...
static void genGetIOB(const char* name, int gm_type, gm_code_writer& Body, gm_giraphlib* lib)
{
    ...
    Body.push(" = in.");
    switch(gm_type) {
        case GMTYPE_INT:    Body.push("readInt()"); break;
        case GMTYPE_LONG:   Body.push("readLong()"); break;
        case GMTYPE_FLOAT:  Body.push("readFloat()"); break;
        case GMTYPE_DOUBLE: Body.push("readDouble()"); break;
        case GMTYPE_BOOL:   Body.push("readBoolean()"); break;
        case GMTYPE_NODE:
...
void gm_giraphlib::generate_master_class_details(
            ...
            gm_code_writer& Body)
{
    ...
    for(I=prop.begin(); I!=prop.end(); I++)
    {
        ...
        genPutIOB(sym->getId()->get_genname(),
                sym->getType()->getTypeSummary(), Body, this);
    }
    Body.pushln("}");
...
void gm_giraphlib::generate_vertex_prop_class_details(
            std::set<gm_symtab_entry* >& prop,
            gm_code_writer& Body, bool is_edge_prop)
{
    ...
    if (FE.get_current_proc_info()->find_info_bool(GPS_FLAG_USE_REVERSE_EDGE)) {
    	...
    	sprintf(temp, "%s = new %s[_node_count];",
    	    GPS_REV_NODE_ID, PREGEL_BE->get_lib()->is_node_type_int() ? "IntWritable" : "LongWritable");
    	Body.pushln(temp);
...
const char* gm_giraphlib::get_message_field_var_name(
        ...
        gm_code_writer& Body, gm_gps_basic_block* bb)
{
    ...
    if (bb->get_type() == GM_GPS_BBTYPE_PREPARE1) {
        ...
        Body.pushln("_msg = _msgs.next();");
            ...
            sprintf(temp,"%s.%s[i] = new %s(%s);", STATE_SHORT_CUT, GPS_REV_NODE_ID,
