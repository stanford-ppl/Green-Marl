...
class giraph_flat_symbol_t : public gm_apply
{
public:
    giraph_flat_symbol_t(
    ...
    virtual bool apply(gm_symtab_entry* sym, int symtab_type)
    {
        ...
        giraph_syminfo* syminfo = (giraph_syminfo*) info;
...
static int comp_start_byte(std::set<gm_symtab_entry*>& prop)
{
    ...
    for(I=prop.begin(); I!=prop.end(); I++)
    {
        ...
        int size = GIRAPH_BE.get_lib()->get_type_size(sym->getType()->get_target_type());
...
void gm_giraph_opt_analyze_symbol_summary::process(ast_procdef* p)
{
    ...
    giraph_flat_symbol_t T( beinfo->get_scalar_symbols(), prop, e_prop); 
    ...
    proc->add_info_bool(GIRAPH_FLAG_NODE_VALUE_INIT, need_node_prop_init);
