...
#define FOR_SEQ1    1
#define FOR_PAR2    2
class check_usage_t : public giraph_apply_bb_ast 
{
public:
    check_usage_t() {
        set_is_pre(true);
...
void check_and_merge_bb_main(giraph_bb* par1, giraph_bb* seq1, giraph_bb* par2, giraph_bb* seq2)
{
    ...
    if (T.is_okay_to_merge()) {
        ...
        if (seq2->get_num_exits() == 1) {
            giraph_bb* next = seq2->get_nth_exit(0);
            assert(next != NULL);
            /*
...
static void find_pattern_and_merge_bb(std::list<giraph_bb*>& current_list)
{
    ...
    // reverse iteration
    std::list<giraph_bb*>::reverse_iterator I;
    for(I=current_list.rbegin(); I!=current_list.rend();I++)
    {
        ...
        if (seq2 == NULL) {
            if (curr->get_type() != GM_GIRAPH_BBTYPE_SEQ) {
                continue;
            }
            else {
                seq2 = curr; continue;
            ...
            if (curr->get_type() == GM_GIRAPH_BBTYPE_BEGIN_VERTEX) {
                par2 = curr;
                continue;
            }
            else if (curr->get_type() == GM_GIRAPH_BBTYPE_SEQ) {
...
static void find_linear_segments(
        giraph_bb* current, 
        std::list<giraph_bb*>& current_list, 
        std::list< std::list<giraph_bb*> >& all_lists,
        std::set<giraph_bb*>& visited)
{
    ...
    if (continue_current_list) {
        ...
        for(int i=0;i<current->get_num_exits();i++) {
            giraph_bb* next = current->get_nth_exit(i);
            std::list<giraph_bb*> new_list;
            find_linear_segments(next, new_list, all_lists, visited);
        }
    }
}

void gm_giraph_opt_merge_ebb_again::process(ast_procdef* p)
{
    ...
    // Apply State Merge
    //-------------------------------------------
    std::list< std::list<giraph_bb*> >::iterator L;
    for(L= all_lists.begin(); L!=all_lists.end(); L++)
    {
        std::list<giraph_bb*>& CL = *L;
