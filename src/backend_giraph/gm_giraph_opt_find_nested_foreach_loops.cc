...
class giraph_opt_find_nested_loops_t : public gm_apply
{

public:
    giraph_opt_find_nested_loops_t(std::map<ast_foreach*, ast_foreach*>& M) : MAP(M)
    {
       set_for_sent(true);
...
extern void gm_giraph_find_double_nested_loops(std::map<ast_foreach*, ast_foreach*>& MAP);

void gm_giraph_find_double_nested_loops(ast_node* p, std::map<ast_foreach*, ast_foreach*>& MAP)
{
    giraph_opt_find_nested_loops_t T(MAP);
    p->traverse_both(&T);
}

/*
void gm_giraph_opt_find_nested_loops_test::process(ast_procdef* p)
{
    std::map<ast_foreach*, ast_foreach*> MAP;
    gm_giraph_find_nested_loops(p, MAP);
