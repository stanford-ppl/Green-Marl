...
struct giraph_intra_merge_candidate_t
{
    giraph_intra_merge_candidate_t() {
        while_cond = par1 = seq1 = parn = seqn = seq0 = NULL;
    }
    giraph_bb* while_cond;
    giraph_bb* seq0;
    giraph_bb* par1;
    giraph_bb* seq1;
    giraph_bb* parn;
...
static void find_linear_while_segments(giraph_bb* entry, std::list<giraph_intra_merge_candidate_t*>& L);
static void apply_intra_merge(giraph_intra_merge_candidate_t*);
void gm_giraph_opt_merge_ebb_intra_loop::process(ast_procdef* p)
{
    ...
    //giraph_bb_print_all(entry);

    //-------------------------------------------
    // find linear (do-)whlie segments
    //-------------------------------------------
    std::list<giraph_intra_merge_candidate_t*> L;
    find_linear_while_segments(entry,L);

    std::list<giraph_intra_merge_candidate_t*>::iterator I;

    /*
    for(I=L.begin(); I!=L.end(); I++) {
        giraph_intra_merge_candidate_t* C = *I;
...
class giraph_find_intra_merge_candidate_t : public giraph_apply_bb {
 public:
    giraph_find_intra_merge_candidate_t(std::list<giraph_intra_merge_candidate_t*>& L) : cands(L)
    {
    ...
    {
        //printf("visiting :%d\n", b->get_id());
        if (b->has_info(GIRAPH_FLAG_WHILE_HEAD))  {
            start_new_trace(b->get_id(), b);
        }
        else if (b->has_info(GIRAPH_FLAG_WHILE_TAIL)) {
            int head_id = b->find_info_int(GIRAPH_FLAG_WHILE_TAIL);
 ...
 private:
    void start_new_trace(int id, giraph_bb* b) { 
        stack.clear();
        current_trace_head = id;
        curr_head = b;
        //printf("start trace : %d\n", id);
    }
    void check_and_finish_trace(int id, giraph_bb* b) { 
        curr_tail = b;
        //printf("end trace : %d\n", id);
        ...
        if (id == current_trace_head) {
            ...
            if (stack.size() >= 4) {
                ...
                else if (p2->find_info_bool(GIRAPH_FLAG_HAS_COMMUNICATION) ||
                         p2->find_info_bool(GIRAPH_FLAG_HAS_COMMUNICATION_RANDOM)) is_okay = false;

                gm_rwinfo_sets* rwi = NULL;
                if (is_okay) {
                    rwi = new gm_rwinfo_sets();
                    gm_giraph_get_rwinfo_from_bb(p1, rwi);

                    gm_rwinfo_sets* rwi_n = new gm_rwinfo_sets();
                    gm_giraph_get_rwinfo_from_bb(s2, rwi_n);
                ...
                if (is_okay) {
                    gm_giraph_get_rwinfo_from_bb(s1, rwi);
                    /*
                ...
                if (is_okay) {
                    ...
                    gm_giraph_get_rwinfo_from_all_reachable_bb(next, rwi2);
                ...
                if (is_okay) {
                    giraph_intra_merge_candidate_t* C = 
                        new giraph_intra_merge_candidate_t();
                    if (curr_head->get_num_exits() > 1)  // WHILE
    ...
    std::list<giraph_intra_merge_candidate_t*>& cands;
    std::list<giraph_bb*> stack;
...
static void find_linear_while_segments(giraph_bb* entry, std::list<giraph_intra_merge_candidate_t*>& L)
{
    ...
    giraph_find_intra_merge_candidate_t T(L);
    giraph_bb_apply_only_once(entry, &T);  // in DFS order
}

static void apply_intra_merge(giraph_intra_merge_candidate_t* C)
{
    // appl
    giraph_bb* p_1 = C->par1;
    giraph_bb* s_1 = C->seq1;
    giraph_bb* p_n = C->parn;
    giraph_bb* s_n = C->seqn;
    giraph_bb* s_0 = C->seq0;
    giraph_bb* while_cond = C->while_cond;
    ...
    for(I = p_n->get_sents().begin(); I!= p_n->get_sents().end(); I++)  
        (*I)->add_info_bool(GIRAPH_FLAG_IS_INTRA_MERGED_CONDITIONAL, true);
    for(I = s_n->get_sents().begin(); I!= s_n->get_sents().end(); I++)  
        (*I)->add_info_bool(GIRAPH_FLAG_IS_INTRA_MERGED_CONDITIONAL, true);

    /* // don't need this
    std::list<gm_giraph_comm_unit>::iterator J;
    for(J = p_n->get_receivers().begin(); J!= p_n->get_receivers().end(); J++)  
    {
        gm_giraph_comm_unit& U = *J;
        if (U.fe != NULL) {
            U.fe->add_info_bool(GIRAPH_FLAG_IS_INTRA_MERGED_CONDITIONAL, true);
        }
        else if (U.sb != NULL) {
            U.sb->add_info_bool(GIRAPH_FLAG_IS_INTRA_MERGED_CONDITIONAL, true);
        }
    }
    */
    ...
    // add tags 
    p_n->add_info_bool(GIRAPH_FLAG_IS_INTRA_MERGED_CONDITIONAL, true);
    p_n->add_info_int(GIRAPH_INT_INTRA_MERGED_CONDITIONAL_NO, while_cond->get_id());
    s_n->add_info_bool(GIRAPH_FLAG_IS_INTRA_MERGED_CONDITIONAL, true);
    s_n->add_info_int(GIRAPH_INT_INTRA_MERGED_CONDITIONAL_NO, while_cond->get_id());


    // create new state
    gm_giraph_beinfo* BEINFO =  (gm_giraph_beinfo*) FE.get_current_backend_info();
    giraph_bb* TAIL2 = new giraph_bb (BEINFO->issue_basicblock_id(), GM_GIRAPH_BBTYPE_MERGED_TAIL);

    // add tag that this is merged conditional
    TAIL2->add_info_int(GIRAPH_INT_INTRA_MERGED_CONDITIONAL_NO, while_cond->get_id());
    while_cond->add_info_bool(GIRAPH_FLAG_IS_INTRA_MERGED_CONDITIONAL, true);
    FE.get_current_proc()->add_info_list_element(GIRAPH_LIST_INTRA_MERGED_CONDITIONAL, while_cond);
    ...
    if (s_0 != NULL) {
        ...
        giraph_bb* while_entry = p_1->get_nth_entry(0);
    ...
    giraph_bb* org_exit = s_n->get_nth_exit(0);
    giraph_bb* p_2 = s_1->get_nth_exit(0);  // p_2 may be p_n
