...
// check condition 1-4
class giraph_check_synth_t : public gm_apply 
{
public:
    giraph_check_synth_t(ast_procdef* p)  {
        _error = false;
        set_for_symtab(true);
        set_for_sent(true);
    ...
    virtual bool apply(ast_sent *s) 
    {
        if (s->get_nodetype() == AST_BFS) {
            gm_backend_error(GM_ERROR_GIRAPH_UNSUPPORTED_OP, 
    ...
    virtual bool apply(gm_symtab_entry* e, int symtab_type)
    {
        ...
        if (gm_is_collection_type(type_id)) {
            gm_backend_error(
                    GM_ERROR_GIRAPH_UNSUPPORTED_COLLECTION,
                    e->getId()->get_line(),
                    e->getId()->get_col(),
                    e->getId()->get_orgname());
            _error = true;
            ...
            _error = true;
            */
            proc->add_info_bool(GIRAPH_FLAG_USE_EDGE_PROP, true);
        }
        ...
        else if (gm_is_graph_type(type_id))
        {
            if (_graph_defined) {
                gm_backend_error(
                    GM_ERROR_GIRAPH_MULTIPLE_GRAPH,
                    e->getId()->get_line(),
                    e->getId()->get_col(),
                    e->getId()->get_orgname());
                _error = true;
...
class giraph_check_synth2_t : public gm_apply 
{
public:
    giraph_check_synth2_t()  {
        _error = false;
        _rand_used = false;
        set_for_expr(true);
    }
...
void gm_giraph_opt_check_synthesizable::process(ast_procdef* proc)
{
    //----------------------------------
    // check condition (1) to (4)
    //----------------------------------
    giraph_check_synth_t T(proc);
