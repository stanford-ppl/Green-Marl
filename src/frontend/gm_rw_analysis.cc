...
static bool is_wider_entry(gm_rwinfo* old, gm_rwinfo * neo) {
    ...
    if (old->access_range != neo->access_range) return false;
    if (old->driver != neo->driver) return false;
    if ((old->always == false) && (neo->always == true)) return true;
    return false;
}
//---------------------------------------------------
// true if error
...
static bool is_reduce_error(gm_rwinfo* old, gm_rwinfo* neo) {
    ...
    if (old->reduce_op != neo->reduce_op) {
        ...
        gm_type_error(GM_ERROR_DOUBLE_BOUND_OP, neo->location->get_line(), neo->location->get_col(), gm_get_reduce_string(old->reduce_op));
        return true;
    }
    ...
    if (old->bound_symbol != neo->bound_symbol) {
        // generate error message
        gm_type_error(GM_ERROR_DOUBLE_BOUND_ITOR, neo->location->get_line(), neo->location->get_col(), old->bound_symbol->getId()->get_orgname());
        return true;
    }
...
//   false; error
//-----------------------------------------------------------
bool gm_add_rwinfo_to_set(gm_rwinfo_map& info_set, gm_symtab_entry* sym, gm_rwinfo* new_entry, bool is_reduce_ops) {
    bool is_error = false;
    ...
    if (i == info_set.end()) {   // not found --> add new;
        ...
        info_set[sym] = l;
    } else {  // check entries already exists
...
void traverse_expr_for_readset_adding(ast_expr* e, gm_rwinfo_map& rset, temp_map_t& DrvMap = Default_DriverMap);

void traverse_expr_for_readset_adding_field(ast_expr* e, gm_rwinfo_map& rset, temp_map_t& DrvMap) {
    ...
    if (i == DrvMap.end()) {
        new_entry = gm_rwinfo::new_field_inst(iter_sym, e->get_field()->get_first()); // iterator syminfo
...
void traverse_expr_for_readset_adding_builtin(ast_expr_builtin* builtin, gm_rwinfo_map& rset, temp_map_t& DrvMap) {
    // add every arguments in the readset
    std::list<ast_expr*>& args = builtin->get_args();
...
void traverse_expr_for_readset_adding_reduce(ast_expr_reduce* e2, gm_rwinfo_map& rset, temp_map_t& DrvMap) {
...
void traverse_expr_for_readset_adding(ast_expr* e, gm_rwinfo_map& rset, temp_map_t& DrvMap) {
    ...
    switch (e->get_opclass()) {
        ...
        case GMEXPR_REDUCE:
            ...
            break;
        case GMEXPR_BUILTIN:  // built-ins does not read or modify anything other than arguments
...
bool gm_rw_analysis::apply_assign(ast_assign *a) {
    ...
    if (a->get_lhs_type() == GMASSIGN_LHS_SCALA) {
        ...
        if (iter_sym->getType()->is_graph()) {
            ...
            new_entry = gm_rwinfo::new_range_inst(GM_RANGE_LINEAR, true, a->get_lhs_field()->get_first());
        } else {
...
static bool merge_for_if(gm_rwinfo_map& Target, gm_rwinfo_map& S1, bool is_reduce) {
    bool is_okay = true;
    gm_rwinfo_map::iterator s1;
...
// return: is_okay
//-----------------------------------------------------------------------------
static bool merge_for_sentblock(ast_sentblock* s, gm_rwinfo_map& target, gm_rwinfo_map& old, bool is_reduce) {
    bool is_okay = true;
    gm_rwinfo_map::iterator i;
...
static bool merge_body(gm_rwinfo_map& R, // top
        gm_rwinfo_map& W, gm_rwinfo_map& D, gm_rwinfo_map& M, ast_sent* s, // body sentence
        bool is_conditional) {
...
static bool cleanup_iterator_access_reduce(ast_id* iter, gm_rwinfo_map& D_temp,  // reduce map of the body
...
private:
    ...
    void print_set(const char* c, gm_rwinfo_map& m) {
        ...
        for (it = m.begin(); it != m.end(); it++, cnt2++) {
            ...
            if (e->getType()->is_property())
                printf("{%s(%s):", e->getId()->get_orgname(), e->getType()->get_target_graph_id()->get_orgname());
            else
...
void gm_rwinfo::print() {
    ...
    if (bound_symbol != NULL) {
        printf(" ,%s, %s ", gm_get_reduce_string(reduce_op), bound_symbol->getId()->get_orgname());
    }
