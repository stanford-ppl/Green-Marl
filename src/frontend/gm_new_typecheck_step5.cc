...
public:
    ...
    virtual bool apply(ast_sent* s) {
        ...
        switch (s->get_nodetype()) {
            ...
            case AST_RETURN: {
                ...
                if (gm_is_void_type(summary_lhs)) break;

                if (r->get_expr() == NULL) {
                    gm_type_error(GM_ERROR_RETURN_MISMATCH, r->get_line(), r->get_col(), gm_get_type_string(summary_lhs), gm_get_type_string(GMTYPE_VOID));
                    break;
                }
                ...
                if (!gm_is_compatible_type_for_assign(summary_lhs, summary_rhs, coed, warn)) {
                    gm_type_error(GM_ERROR_RETURN_MISMATCH, r->get_line(), r->get_col(), gm_get_type_string(summary_lhs), gm_get_type_string(summary_rhs));
    ...
    bool check_assign_lhs_rhs(ast_node* lhs, ast_expr* rhs, int l, int c) {
        ...
        int coed;
        if (!gm_is_compatible_type_for_assign(summary_lhs, summary_rhs, coed, warn)) {
            gm_type_error(GM_ERROR_ASSIGN_TYPE_MISMATCH, l, c, gm_get_type_string(summary_lhs), gm_get_type_string(summary_rhs));
            return false;
        }
        if (warn && gm_is_prim_type(summary_lhs)) {
            printf("warning: adding type conversion %s->%s\n", gm_get_type_string(summary_rhs), gm_get_type_string(summary_lhs));
